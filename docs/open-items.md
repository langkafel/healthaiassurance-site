...
| offen | **`app.healthaiassurance.de`-vHost in Coolify** | Der Name löst auf die Server-IP auf, liefert aber 404 bzw. ein nicht vertrauenswürdiges Zertifikat, weil der App-Service dort keine Domain zugewiesen hat. Betrifft die App, nicht dieses Repo — die Seite verlinkt bewusst nicht mehr dorthin (Nav und `/loesung` → interne Seite `/login`, „Bald verfügbar“). Sobald die Plattform live ist, `/login` durch den Link auf `https://app.healthaiassurance.de` ersetzen. |
...
| erledigt | `/embeds/*.html` tragen `<meta name="robots" content="noindex, nofollow">` (per `build-embeds.mjs`); bewusst **kein** `Disallow` in `robots.txt`, damit Crawler das noindex sehen. Weiterhin direkt aufrufbar, aber ohne Nav/Footer. |
